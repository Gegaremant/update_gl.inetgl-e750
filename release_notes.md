...
The GL-E750 router contains two separate systems: the main CPU running OpenWrt and a separate Microcontroller Unit (MCU) controlling the OLED display. This daemon (`gl-e750-mcu`) runs on the OpenWrt system and acts as a bridge. It listens to the system state via `ubus` and talks to the MCU via UART serial port (`/dev/ttyS0`), sending it JSON-formatted instructions on what to display.
...
### How to upgrade the MCU firmware natively
*(Note: This is for flashing the microcontroller chip itself, not the OpenWrt daemon package)*
...
Роутер GL-E750 (Mudi) состоит из двух независимых систем: главного процессора с OpenWrt и отдельного микроконтроллера (MCU), который управляет OLED-экраном. Утилита `gl-e750-mcu` работает в OpenWrt и является мостом: она собирает информацию о состоянии системы (через шину `ubus`) и отправляет её микроконтроллеру через последовательный порт (`/dev/ttyS0`) в формате JSON. Микроконтроллер читает этот JSON и отрисовывает нужные пиксели на экране.
...
### Пример 1: Управление OLED дисплеем вручную
...
{OK},99,42.4,1,2
```
### Пример 2: Как отладить процесс
Если вы хотите проверить версию прошивки MCU или просмотреть логи:
...
### Как обновить саму прошивку микроконтроллера (MCU)
*(Примечание: это инструкция по перепрошивке самого чипа экрана `.bin` файлом, а не пакета дисплея для OpenWrt).*
